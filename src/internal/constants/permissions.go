...
// 後方互換性のためのエイリアス
const (
	USER_WRITE               = USER_UPDATE // 旧名称との互換性
	USER_APPROVE             = USER_UPDATE // ユーザー承認もUSER_UPDATEに含まれる
	PROFILE_READ             = USER_READ   // プロフィール読み取りはUSER_READに含まれる
	PROFILE_WRITE            = USER_UPDATE // プロフィール編集はUSER_UPDATEに含まれる
	EXTERNAL_IDENTITY_READ   = USER_READ   // 外部ID閲覧はUSER_READに含まれる
	EXTERNAL_IDENTITY_WRITE  = USER_UPDATE // 外部ID編集はUSER_UPDATEに含まれる
	EXTERNAL_IDENTITY_DELETE = USER_UPDATE // 外部ID削除はUSER_UPDATEに含まれる
	AUDIT_LOG_READ           = AUDIT_READ  // 監査ログ読み取り
)
